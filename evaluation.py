...
def evaluation(cfg_path, model_path, eval_type):
    ...
    cfg = utils.utils.load_datafile(cfg_path)
    ...
    val_dataset = utils.datasets.TensorDatasetSp(cfg["val"], cfg["width"], cfg["height"], cfg["label_flag"], imgaug=False)
    ...
    _, _, AP, _ = utils.utils.evaluation(val_dataloader, cfg, model, device, cfg["conf_thr"], nms_thresh=cfg["nms_thr"],
                                      iou_thres=cfg["iou_thr"], one_norm=one_norm)
    print("computer PR...")
    # modified
    # precision, recall, _, f1 = utils.utils.evaluation(val_dataloader, cfg, model, device, 0.3)
    precision, recall, _, f1 = utils.utils.evaluation(val_dataloader, cfg, model, device, 0.3, nms_thresh=cfg["nms_thr"],
                                                   iou_thres=cfg["iou_thr"], one_norm=one_norm)
