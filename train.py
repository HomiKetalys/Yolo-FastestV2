...
def train(opt,log_and_save):
    ...
    train_dataset = datasets.TensorDatasetSp(cfg["train"], cfg["width"], cfg["height"],cfg["label_flag"], imgaug=True)
    val_dataset = datasets.TensorDatasetSp(cfg["val"], cfg["width"], cfg["height"],cfg["label_flag"], imgaug=False)
    ...
    model = detector.DetectorSp(cfg["classes"], cfg["anchor_num"], load_param, separation=cfg["separation"],
    ...
    for epoch in range(cfg["epochs"]):
        ...
        for imgs, targets in pbar:
            ...
            iou_loss, obj_loss, cls_loss, total_loss = loss.compute_loss(preds, targets, cfg, device)
        ...
        if epoch % 10 == 0 and epoch>0:
            ...
            _, _, AP, _ = utils.evaluation(val_dataloader, cfg, model, device,cfg["conf_thr"],nms_thresh=cfg["nms_thr"],iou_thres=cfg["iou_thr"])
            print("computer PR...")
            # modified
            # precision, recall, _, f1 = utils.utils.evaluation(val_dataloader, cfg, model, device, 0.3)
            precision, recall, _, f1 = utils.evaluation(val_dataloader, cfg, model, device, 0.3,nms_thresh=cfg["nms_thr"],iou_thres=cfg["iou_thr"])
            # modified
